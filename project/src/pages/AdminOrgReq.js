...
function AdminOrgReq() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', padding: '20px' }}>
      {popupMessage && (
        <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', padding: '20px', border: '1px solid black', borderRadius: '5px', zIndex: 1 }}>
          {popupMessage}
        </div>
      )}
      <h1 style={{ gridColumn: '1/-1', textAlign: 'center' }}>Pending Requesting Organizations</h1>
      {donors.map((donor, index) => (
        !donor.accepted && !donor.rejected && (
          <Box key={index} component="section" sx={{ p: 2, border: '1px dashed grey', borderRadius: '8px', boxShadow: '0px 4px 10px rgba(0, 0, 0, 0.1)' }}>
            <h2>{donor.organization}</h2>
            <p><strong>Name:</strong> {donor.firstName} {donor.lastName}</p>
            <p><strong>Gender:</strong> {donor.gender}</p>
            <p><strong>Organization Name:</strong> {donor.organizationname}</p>
            <p><strong>Organization Type:</strong> {donor.organizationtype}</p>
            ...
            <div style={{ marginTop: '20px', textAlign: 'center' }}>
              <button onClick={() => handleAccept(donor)} style={{ marginRight: '10px', backgroundColor: '#4CAF50', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer' }}>Accept</button>
              <button onClick={() => handleReject(donor)} style={{ backgroundColor: '#f44336', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer' }}>Reject</button>
            </div>
            <div style={{ marginTop: '10px', textAlign: 'center' }}>
              <a href={Org} download style={{ textDecoration: 'none', backgroundColor: '#008CBA', color: 'white', padding: '10px 20px', borderRadius: '5px', cursor: 'pointer' }}>Download Certificate</a>
            </div>
          </Box>
        )
      ))}
